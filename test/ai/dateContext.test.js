...
test("Asia/Seoul(KST) 기준 오늘 다음 날부터 15일 예약 가능 범위를 계산한다", () => {
  ...
  assert.equal(context.windowEnd, "2026-09-19");
});
...
test("KST 자정 경계에서 날짜가 정확히 전환된다", () => {
  ...
  assert.equal(beforeMidnight.windowEnd, "2026-09-19");
  ...
  assert.equal(afterMidnight.windowEnd, "2026-09-20");
});
...
test("월말 및 연말 경계에서 예약 가능 기간이 올바르게 계산된다", () => {
  ...
  assert.equal(yearEnd.windowEnd, "2027-01-09");
...
test("buildChatSystemPrompt는 기본 지시사항과 명시적 KST 날짜 규칙을 포함한다", () => {
  ...
  assert.ok(prompt.includes("예약 가능 범위는 2026-09-05 ~ 2026-09-19입니다."));
  assert.ok(
    prompt.includes(
