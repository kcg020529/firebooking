...
AI가 직접 커밋해도 된다. 단 **커밋에 남는 작성자는 실제 작업자 한 사람뿐이다.**

- `Co-Authored-By` 같은 AI 공동 작성자 트레일러를 붙이지 않는다
- 세션 링크·생성 도구 표시를 커밋 메시지에 넣지 않는다
- `--author`를 바꾸지 않는다. 커밋은 그 브랜치를 맡은 본인 계정으로만 남는다
