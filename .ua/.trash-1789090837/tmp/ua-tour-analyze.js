...
try {
  ...
  for (const [id, neighbors] of adjacency) for (const target of neighbors) if (id < target && adjacency.get(target)?.has(id)) bidirectionalPairs.push([id, target]);
  const clusters = bidirectionalPairs.map((pair) => {
    const members = new Set(pair); let changed = true;
    while (changed && members.size < 5) {
      changed = false;
      for (const [id, neighbors] of adjacency) {
        if (members.has(id)) continue;
        let links = 0; for (const member of members) if (neighbors.has(member) || adjacency.get(member)?.has(id)) links++;
        if (links >= 2 && members.size < 5) { members.add(id); changed = true; }
      }
    }
    const ids = [...members].sort(); let edgeCount = 0;
