...
export function sanitizeReviewContent(content) {
  ...
  // 자유 문장인 리뷰에서는 2~4글자 한글 전체를 이름으로 단정할 수 없다.
  // 단독 이름 규칙만 끄고, 이름 문맥 및 전화·이메일·주민번호·카드 탐지는 유지한다.
  return detectAndMaskPii(text, { detectStandaloneName: false });
}

export function getFeaturedReviews(reviews = []) {
  return [...reviews].sort((a, b) => (b.likeCount ?? 0) - (a.likeCount ?? 0)).slice(0, 1);
}
...
export async function deleteCourseReview(courseId, reviewId, userId, { client = createServerClient(), isAdmin = false } = {}) {
  let query = client.from('course_reviews').delete().eq('id', reviewId).eq('course_id', courseId);
