...
import { encryptIncidentIp, getIncidentIpExpiry } from './hash.js';
...
export async function recordSecurityEvents(events, options = {}) {
  ...
  const rows = list.map((event) => {
    const row = { ...event };
    if (event.severity !== 'critical' || !networkContext?.ip) return row;

    try {
      row.ip_ciphertext = encryptIncidentIp(networkContext.ip, encryptionEnv);
      row.ip_expires_at = getIncidentIpExpiry(event.ts ?? new Date());
    } catch {
      // 암호화 설정 오류가 원본 보안 이벤트 저장과 Discord 알림을 막지 않는다.
      console.error('[securityEvents] 사고 IP 암호화 실패:', {
        ruleId: event.rule_id || event.ruleId,
        code: 'IP_ENCRYPTION_FAILED',
      });
    }
    return row;
  });
  ...
  // 1. 기존 보안 이벤트 DB 영속화
  if (fetchImpl) {
    await insertRestRows('security_events', rows, fetchImpl);
  } else {
  ...
  for (const item of list) {
    if (item.severity === 'critical') {
      try {
        await sendCriticalSecurityAlert(item, {
          ...alertOptions,
          networkContext: alertOptions?.networkContext ?? networkContext,
        });
      } catch {
