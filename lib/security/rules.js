...
/**
 * 최근 windowMinutes 안에 같은 IP 가 남긴 특정 결과의 감사 기록 수를 센다.
 * result 기본값은 'deny'(실패 조회 카운트). 스캘핑 차단은 'allow'(성공 예약)를 센다.
 *
 * audit_logs 를 그대로 근거로 쓴다 — 탐지를 위해 별도 카운터 테이블을
 * 두면 그 테이블이 또 관리 대상이 되고, 감사 기록과 어긋날 수 있다.
 */
async function countRecentDenies(supabase, { ipHash, action, windowMinutes, result = 'deny' }) {
...
async function insertAnomalyOnce(
  supabase,
  { rule, ipHash, actorId, evidence, networkContext = null },
) {
 ...
 * 슬롯 선점(스캘핑) 차단.
 *
 * 예약 생성 "전에" 동기로 부른다. 같은 IP 가 짧은 시간에 예약을 쓸어담아
 * 슬롯을 고갈시키는 것을 막는다. 임계값을 넘으면 예약을 만들지 않고
 * ANO_SCALP 이벤트를 남긴 뒤 차단한다. (detectScalping 은 사후 기록,
 * 이 함수는 사전 차단 — 예약 라우트가 이걸 먼저 통과해야 createBooking 에 닿는다)
 *
 * 저장소 조회가 실패하면 서비스는 막지 않는다. 로그 장애가 예약 자체를
 * 세우면 안 되기 때문. (요청 제한과 같은 원칙)
 */
export async function checkBookingScalp({ ipHash, actorId = null, networkContext = null, client } = {}) {
  if (!ipHash) return { limited: false };
  ...
  try {
    const supabase = client ?? createServerClient();
    const attempts = await countRecentDenies(supabase, {
      ipHash,
    ...
    if (attempts < rule.threshold) return { limited: false };

    await insertAnomalyOnce(supabase, {
      rule,
      ipHash,
      actorId,
      evidence: `${rule.windowMinutes}분 내 예약 생성 ${attempts}회로 추가 예약 차단`,
      networkContext,
    });
    return { limited: true, retryAfterSeconds: rule.windowMinutes * 60 };
  } catch (error) {
    console.error('[rules] ANO_SCALP 차단 검사 실패:', error);
    return { limited: false };
  }
}
...
export function detectCodeEnumeration(
  { ipHash, actorId, action, networkContext = null },
...
export function detectLookupBruteForce(
  { ipHash, actorId, action, phoneHash, networkContext = null },
  { client, schedule = after } = {},
) {
