...
import { createMaskedPathEvidence } from '@/lib/security/pii';
 ...
 */
export async function recordUnauthorizedAdminAccess({ path, user, targetType = 'page' }) {
  const ipHash = await getIpHashFromHeaders();

  const actorId = user?.id ?? null;
  const actorRole = user?.role ?? 'guest';
  const safePath = createMaskedPathEvidence(path, 160);
  ...
  after(async () => {
    try {
      ...
      await supabase.from('security_events').insert({
        ...
        evidence: `${actorRole} 역할이 ${safePath} 접근 시도`,
      });
...
export async function recordUnknownAdminPath({ path, user }) {
  const ipHash = await getIpHashFromHeaders();
  const safePath = createMaskedPathEvidence(path, 160);
  ...
  after(async () => {
    try {
      ...
      await supabase.from('security_events').insert({
        ...
        ip_hash: ipHash,
        evidence: `존재하지 않는 관리자 경로 접근: ${safePath}`,
      });
    } catch (error) {
