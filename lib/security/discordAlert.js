 ...
 * critical 사고 대응을 위해 요청 시점의 IP·국가·경로를 포함한다.
 * actor_id와 인증 토큰은 포함하지 않는다.
 *
 * @param {object} event
...
export function buildDiscordPayload(event, options = {}) {
  ...
  if (networkContext.ip) {
    fields.push({ name: '원본 IP', value: String(networkContext.ip).slice(0, 80), inline: true });
  }
  if (networkContext.country) {
    fields.push({ name: '국가', value: String(networkContext.country).slice(0, 16), inline: true });
  }
  if (event.ip_hash || event.ipHash) {
    fields.push({
      name: 'IP 해시',
      value: String(event.ip_hash || event.ipHash).slice(0, 64),
      inline: true,
    });
  }
  if (networkContext.path) {
    const requestSummary = `${networkContext.method || 'UNKNOWN'} ${networkContext.path}`;
    fields.push({ name: '요청', value: sanitizeEvidence(requestSummary, options.env), inline: false });
  }
  if (networkContext.userAgent) {
    fields.push({
      name: 'User-Agent',
      value: sanitizeEvidence(String(networkContext.userAgent), options.env).slice(0, 300),
      inline: false,
    });
  }
 ...
 * @param {object} [options.networkContext] - 원본 IP·국가·경로·User-Agent (critical 전용)
...
export async function sendCriticalSecurityAlert(event, options = {}) {
  ...
  const sourceFingerprint = event.ip_hash
    || event.ipHash
    || options.networkContext?.ip
    || 'unknown';
  const cacheKey = `${ruleId}:${sourceFingerprint}:${maskedEvidence}`;
