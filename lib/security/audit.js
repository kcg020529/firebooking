...
export const AUDIT_ACTIONS = {
  ...
  SECURITY_EVENT_HANDLE: 'security.event.handle',
  SECURITY_IP_BLOCK: 'security.ip.block',
  SECURITY_IP_UNBLOCK: 'security.ip.unblock',
  SECURITY_LOGIN_UNLOCK: 'security.login.unlock',
};
 ...
 * @param {(context: {client: object, row: object}) => Promise<void>} [entry.onRecorded]
 *        감사 행 저장 성공 직후 같은 작업에서 실행할 탐지
 */
export function recordAudit(request, entry) {
  const row = {
    ...
    target_id: entry.targetId
      ? entry.targetId.startsWith('/')
        ? createMaskedPathEvidence(entry.targetId, 160)
        : createMaskedEvidence(entry.targetId, 160)
      : null,
    result: entry.result,
  ...
  after(async () => {
    try {
      ...
      const storedRow = { ...row, actor_role: role ?? 'guest' };
      const { error } = await supabase.from('audit_logs').insert(storedRow);
      if (error) throw error;

      if (typeof entry.onRecorded === 'function') {
        await entry.onRecorded({ client: supabase, row: storedRow });
      }
    } catch (error) {
