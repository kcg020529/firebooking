...
import { getRuleMetadata } from './securityConfig.js';
...
/**
 * 탐지 이벤트의 처리 상태를 바꾼다.
 * staff·admin 이 확인을 마친 이벤트를 '처리됨'으로, 다시 볼 이벤트를 '미처리'로 되돌린다.
 * 감사 기록은 요청 정보를 가진 라우트가 남긴다.
 */
export async function setSecurityEventHandled({
  eventId,
  handled,
  client = createServerClient(),
}) {
  ...
  if (error) throw error;
  if (!data) {
    return { ok: false, status: 404, error: '보안 이벤트를 찾을 수 없습니다.' };
  }

  return { ok: true, event: { id: data.id, handled: data.handled } };
}
...
function toSecurityEvent(row) {
  const metadata = getRuleMetadata(row.rule_id);
  return {
    id: row.id,
    ts: row.ts,
    ruleId: row.rule_id,
    ...
    handled: row.handled,
    ruleName: metadata?.name ?? row.rule_id,
    eventType: metadata?.eventType ?? 'non_attack',
    criteria: metadata?.criteria ?? '등록되지 않은 규칙',
    response: metadata?.response ?? '운영자 확인',
    canUnlockLogin: row.rule_id === 'ANO_LOGIN_BF' && Boolean(row.response_target_hash),
    // 암호문 자체는 내려보내지 않는다. 보관 만료 시각으로 "조회 가능한 사고 IP가 있다"만 알린다.
    ipExpiresAt: row.ip_expires_at ?? null,
  };
}
...
export async function listSecurityEvents(filters = {}) {
  ...
  let query = supabase
    .from('security_events')
    .select('id, ts, rule_id, category, severity, actor_id, ip_hash, evidence, handled, ip_expires_at, response_target_hash')
  ...
  const events = data.map(toSecurityEvent);
  return filters.eventType
    ? events.filter((event) => event.eventType === filters.eventType)
    : events;
}
...
export async function summarizeSecurityEvents(filters = {}) {
  ...
  const byEventType = { attack: 0, non_attack: 0 };
  const byRule = {};
  let unhandled = 0;

  for (const e of events) {
    if (e.severity in bySeverity) bySeverity[e.severity] += 1;
    if (e.eventType in byEventType) byEventType[e.eventType] += 1;
