 ...
 *   });
 *
 * getUser 는 withApiLog 가 만든 캐시된 조회다. 여기서 새로 부르면
 * 로깅용 조회와 별개로 왕복이 한 번 더 생긴다.
 */
export async function requireStaff(request, getUser) {
  ...
  await recordUnauthorizedAdminAccess({ path, user, targetType: 'api' });
