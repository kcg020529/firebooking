...
function previousAssistantAskedForConfirmation(messages) {
  const previousAssistant = [...messages]
    .reverse()
    .find(({ role }) => role === "assistant");
  return /예약할까요|진행할까요/.test(previousAssistant?.content ?? "");
}

function classifyConfirmationReaction(value) {
  const text = typeof value === "string" ? value.trim() : "";
  if (/안\s*해|취소|그만|하지\s*마/.test(text)) return "cancel";
  if (/^[ㅋㅎ]+$/.test(text) || /^(아\s*)?(씨?발|시발|병신(?:년)?)[\s!.?]*$/.test(text)) {
    return "reaction";
  }
  return null;
}
...
export function createChatService({
  ...
  return async function handleChat({
    ...
    if (previousAssistantAskedForConfirmation(inspection.messages)) {
      const reaction = classifyConfirmationReaction(latestMessage.content);
      if (reaction === "cancel") {
        const reply = "알겠습니다. 예약을 진행하지 않았습니다. 다른 예약을 원하시면 새 조건을 말씀해주세요.";
        return {
          ok: true,
          reply,
          ...withReplySignature(sessionId, reply),
      ...
      if (reaction === "reaction") {
        const reply = "예약은 아직 진행하지 않았습니다. 계속하려면 ‘예약 진행’, 수정하려면 바꿀 내용을 말씀해주세요.";
        return {
          ok: true,
          reply,
          ...withReplySignature(sessionId, reply),
          quickReplies: ["예약 진행", "예약 취소"],
          status: 200,
        };
      }
    }
