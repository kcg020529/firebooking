...
function shapeSearchResult(result) {
  const items = Array.isArray(result) ? result : result?.slots;
...
function shapeBookingResult(result) {
  return pick(result, ["bookingCode"]);
}
...
export async function executeToolCall({ name, input }, dependencies, context = {}) {
  ...
  if (name === "lookup_booking" && !isNonEmptyString(context.sessionId, 200)) {
  ...
  try {
    if (name === "search_slots" && typeof dependencies.searchSlots === "function") {
      ...
      return { ok: true, slots: shapeSearchResult(result) };
    }

    if (name === "create_booking" && typeof dependencies.createBooking === "function") {
      const result = await dependencies.createBooking({ ...input, source: "chat" });
      return { ok: true, ...shapeBookingResult(result) };
    }
    ...
    if (name === "lookup_booking" && typeof dependencies.lookupBooking === "function") {
      ...
      return { ok: true, bookings: shapeLookupResult(result) };
    }
  } catch {
