...
const CONFIRMATION_WORDS = new Set([
  "네", "예", "응", "어", "ㅇㅇ", "넵", "네네", "예예", "좋아", "좋습니다",
  "오케이", "ok", "okay", "ㄱㄱ", "고고", "진행", "진행해", "진행해줘",
  "진행해주세요", "해줘", "해주세요", "예약해", "예약해줘", "예약해주세요",
  "예약진행", "예약진행해줘", "그대로해줘", "이대로해줘", "그렇게해줘",
]);

export function isExplicitBookingConfirmation(value) {
  if (typeof value !== "string") return false;
  if (/(안\s*해|하지\s*마|취소|그만|아니|됐어)/i.test(value)) return false;
  const normalized = value
    .toLocaleLowerCase("ko-KR")
    .replace(/[\s.,!?~ㅋㅎㅜㅠ]+/g, "");
  if (CONFIRMATION_WORDS.has(normalized)) return true;
  return /^(네|예|응|ㅇㅇ|넵)?(오케이|ok|okay|ㄱㄱ|고고|좋아|좋습니다|진행|진행해|진행해줘|진행해주세요|해줘|해주세요|예약해|예약해줘|예약해주세요|예약진행|예약진행해줘|그대로해줘|이대로해줘|그렇게해줘)요?$/.test(normalized);
}
...
export function createDeepSeekGenerator(
  ...
  return async function generateReply({
    ...
    const authInstruction = actorId
      ? "사용자는 로그인 상태입니다."
      : "사용자는 비로그인 상태입니다. 검색과 안내는 가능하지만 예약 생성은 절대 시도하지 말고, 예약하려면 로그인이 필요하다고 안내하세요.";
    const conversation = [
      { role: "system", content: `${buildChatSystemPrompt(clock)}\n\n로그인 상태:\n- ${authInstruction}` },
      ...messages.map(({ role, content }) => ({ role, content })),
    ];
    let bookingCode;
    // 이번 응답에서 search_slots 가 돌려준 slot id. create_booking 은 여기 있는 id 만 쓴다.
    const knownSlotIds = new Set();
    const latestUserMessage = [...messages]
      .reverse()
      .find(({ role }) => role === "user")?.content;
    const allowBookingCreation = isExplicitBookingConfirmation(latestUserMessage);
    ...
    for (let round = 0; round <= MAX_TOOL_ROUNDS; round += 1) {
      ...
      for (const toolCall of toolCalls) {
        ...
        const result = await executeToolCall(
          ...
          toolDependencies,
          { sessionId, actorId, knownSlotIds, allowBookingCreation },
        );
