...
export async function getAuthUser() {
  ...
  const timeoutToken = cookieStore.get(SESSION_TIMEOUT_COOKIE)?.value;
  if (!isSessionTimeoutTokenValid(timeoutToken, user.id)) return null;

  // Route Handler에서 인증된 요청도 활동으로 센다. 서버 컴포넌트처럼
  // 쿠키 쓰기가 금지된 문맥은 proxy가 갱신하므로 조용히 넘긴다.
  try {
    cookieStore.set(
      SESSION_TIMEOUT_COOKIE,
 ...
 *
 * 토큰 검증과 분리해둔 이유: 이미 검증이 끝난 화면에서는 이 조회를
 * 다른 조회(예약 목록 등)와 병렬로 돌릴 수 있다.
 *
 * @returns {Promise<{ role: string, displayName: string | null }>}
 */
