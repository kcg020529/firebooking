...
function toCourse(row) {
  const reviewRows = row.course_reviews ?? [];
  ...
  reviewRows.forEach((review) => { if (difficultyCounts[review.difficulty] !== undefined) difficultyCounts[review.difficulty] += 1; });
  const difficulty = Object.keys(difficultyCounts).reduce((best, level) => difficultyCounts[level] > difficultyCounts[best] ? level : best, 'easy');
  return {
    id: row.id,
    name: row.name,
    type: row.type,
    ...
    averageRating,
    difficulty: reviewRows.length ? difficulty : null,
  };
}
...
export async function listCourses({ type } = {}) {
  ...
  let query = supabase
    .from('courses')
    .select('id, name, type, region, address, phone, image_url, description, course_reviews(rating, difficulty)')
 ...
 * 슬롯 단건 상세 + 골프장 요약 정보.
 *
 * @param {string} id 슬롯 UUID
 * @param {{ client?: object }} [options] 테스트용 Supabase 클라이언트 주입
 * @returns {Promise<object | null>}
 */
export async function getSlot(id, { client } = {}) {
  if (!id) return null;
  ...
  const { data, error } = await supabase
    .from('slots')
    .select(`
      id, date, time, price, capacity, booked,
  ...
  const courseRow = Array.isArray(data.courses) ? data.courses[0] : data.courses;
  const course = courseRow ? toCourseSummary(courseRow) : null;
