...
/** 오늘 날짜(KST, YYYY-MM-DD). slots.date 와 같은 형식이라 문자열 비교가 된다. */
function todayInKst() {
...
/**
 * 같은 전화번호가 이미 들고 있는 미래 예약 수를 센다.
 * 지난 슬롯은 세지 않는다 — 오늘 이후 슬롯만 대상.
 */
async function countActiveBookingsByPhone(supabase, phone) {
  const { count, error } = await supabase
    .from('bookings')
    .select('id, slots!inner(date)', { count: 'exact', head: true })
    .eq('phone', phone)
    .gte('slots.date', todayInKst());
 ...
 *
 * @param {object} input
 * @param {{ client?: object }} [options] 테스트용 Supabase 클라이언트 주입
 * @returns {Promise<{ ok: true, booking: object } | { ok: false, error: string }>}
 */
export async function createBooking(input, { client } = {}) {
  ...
  // 같은 슬롯에 같은 전화번호 예약이 이미 있으면 새로 만들지 않는다.
  // 챗봇은 "네", "감사합니다" 같은 답을 예약 요청으로 오해해 create_booking 을 다시 부를 수 있고,
  // 폼도 두 번 제출될 수 있다. 기존 예약번호는 돌려주지 않는다 — 남의 전화번호로
  // 예약번호를 알아내는 경로가 되기 때문이다.
  try {
    ...
    if (Array.isArray(existing) && existing.length > 0) {
      return {
        ok: false,
        error: '이미 같은 시간에 이 전화번호로 예약이 있습니다. 예약 조회에서 확인해주세요.',
      };
    }
  } catch (error) {
    console.error('[createBooking] 중복 예약 확인 실패:', error);
  }

  // 한 전화번호가 미래 슬롯을 독점해 예약을 고갈시키는 것을 막는다.
  // 폼·챗봇 모두 이 함수를 거치므로 두 경로에 동일하게 적용된다.
  try {
    const activeCount = await countActiveBookingsByPhone(supabase, value.phone);
    if (activeCount >= MAX_ACTIVE_BOOKINGS_PER_PHONE) {
      return {
        ok: false,
        error: `이 전화번호로는 예약을 최대 ${MAX_ACTIVE_BOOKINGS_PER_PHONE}건까지 할 수 있어요. 기존 예약을 확인해주세요.`,
      };
    }
  } catch (error) {
    // 카운트 실패로 정상 예약을 막지는 않는다. 원인은 서버 로그에만 남긴다.
    console.error('[createBooking] 활성 예약 수 확인 실패:', error);
  }
...
function toBookingSummary(row) {
  return {
    bookingCode: row.booking_code,
    partySize: row.party_size,
    memo: row.memo ? detectAndMaskPii(row.memo).maskedText : null,
    source: row.source,
    createdAt: row.created_at,
 ...
 */
export async function lookupBookings({ code, phone }, { client } = {}) {
 ...
 * @param {{ client?: object }} [options] 테스트용 Supabase 클라이언트 주입
 */
export async function listMyBookings(userId, { client } = {}) {
  if (!userId) return [];
...
/** Cancel a booking owned by the authenticated user and release its seats. */
export async function cancelBooking(bookingCode, userId, { client } = {}) {
  const normalizedCode = typeof bookingCode === 'string' ? bookingCode.trim().toUpperCase() : '';
  if (!normalizedCode || !userId) {
    return { ok: false, error: '취소할 예약을 찾을 수 없습니다.' };
  }
  ...
  if (error) {
    ...
    return { ok: false, error: '예약을 취소하지 못했습니다. 잠시 후 다시 시도해주세요.' };
  }
  if (!data) return { ok: false, error: '취소할 예약을 찾을 수 없습니다.' };
  return { ok: true, bookingCode: data };
}
